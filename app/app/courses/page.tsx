...
export default async function CoursesPage({ searchParams }: { searchParams: Promise<{ q?: string; level?: string; semester?: string }> }) {
  ...
  return <div>
    ...
    <div className="page-head"><div><div className="eyebrow">RESULTS</div><h2 style={{color:'var(--primary-navy)',margin:'5px 0'}}>Courses {q ? `for “${q}”` : ''}</h2></div><span className="badge">{courses?.length || 0} found</span></div>
    <div className="grid grid-3">{courses?.map(c=><Link href={`/app/courses/${c.id}`} className="service-card-container" key={c.id}><div style={{display:'flex',justifyContent:'space-between',gap:8}}><strong style={{color:'var(--primary-navy)',fontSize:18}}>{c.code}</strong><span className="badge good">{labels[c.source_type]||c.source_type}</span></div><div style={{marginTop:8,fontWeight:700}}>{c.title}</div><div className="muted" style={{marginTop:6}}>{c.units} unit{c.units===1?'':'s'} · {c.level} Level{c.semester?` · ${c.semester===1?'1st':'2nd'} semester`:''}</div></Link>)}</div>
    {!courses?.length && <div className="notice">No verified course matches that search yet. Try the course code or title, or submit an institution-specific course.</div>}
  </div>
}